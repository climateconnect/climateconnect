...
from organization.tasks import send_event_registration_confirmation_email as _send_registration_email
...
class RegisterForEventView(APIView):
    ...
    def post(self, request, url_slug):
        ...
        if er.max_participants is not None:
            new_count = EventParticipant.objects.filter(
                event_registration=er
            ).count()
        ...
        # ── 8. Dispatch async confirmation email (outside lock is fine; txn
        #        commits before Celery picks it up via transaction.on_commit) ──
