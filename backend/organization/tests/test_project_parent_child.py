...
class ProjectParentChildModelTests(TestCase):
    ...
    def test_prevent_parent_with_existing_children(self):
        ...
        self.assertIn("with child projects cannot have a parent", str(context.exception))
...
class ProjectParentChildAPITests(TestCase):
    ...
    def test_list_serializer_includes_parent_id_and_has_children(self):
        ...
        parent_data = next((p for p in response.data['results'] if p['url_slug'] == 'festival-2026'), None)
        ...
        child_data = next((p for p in response.data['results'] if p['url_slug'] == 'workshop-1'), None)
...
class ProjectParentChildPerformanceTests(TestCase):
    ...
    def test_list_endpoint_query_count(self):
        ...
        self.assertLess(query_count, 15,
                       f"List view has {query_count} queries, expected < 15")
