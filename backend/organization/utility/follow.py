...
from organization.utility.notification import create_organization_follower_notification, create_project_follower_notification
...
def set_user_following(request_data, user, model, url_slug, follower_model, field_name, msgs):
        ...
        if request_data["following"] == True:
            if follower_model.objects.filter(
                user=user, **query
            ).exists():
