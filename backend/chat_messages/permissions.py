...
class ParticipantReadWritePermission(BasePermission):
    def has_permission(self, request, view):
        try:
            chat = MessageParticipants.objects.get(chat_uuid=str(view.kwargs.get('chat_uuid')))
        ...
        if requesting_participant.exists() and participant_to_update.exists(): 
            if requesting_participant[0].id == participant_to_update[0].id:
                if requesting_participant[0].role.role_type == Role.ALL_TYPE and not requesting_participant[0].role.role_type == participant_to_update[0].role.role_type:
                    return False
                else:
                    return True
            if requesting_participant[0].role.role_type > participant_to_update[0].role.role_type:
                return True
        return False
        ...
        if 'chat_participants' in request.data:
            for participant in request.data['chat_participants']:
                if 'permission_type_id' not in participant:
                    return False
                try:
                    new_participant_role = Role.objects.filter(id=int(participant['permission_type_id']))[0]
                except Role.DoesNotExist:
                    return False
                if new_participant_role.role_type >= requesting_participant[0].role.role_type:
                    return False
        return True
