...
def translate_hubs(hubs):
    ...
    for hub in hubs:
        print(hub.url_slug)
        translation = [tn for tn in hub_translations if tn['url_slug'] == hub.url_slug][0]
        if translation:
            HubTranslation.objects.create(
                hub=hub, 
                language=Language.objects.get(language_code=translation['target_language']),
                ...
                image_attribution_translation=translation['image_attribution_translation'],
...
def translate_hub_stats(hub_stats):
    ...
    for hub_stat in hub_stats:
        translation = [tn for tn in hub_stat_translations if tn['name'] == hub_stat.name][0]
        if translation:
            HubStatTranslation.objects.create(
                ...
                value_description_translation=translation['value_description_translation'],
