...
class GetPersonalIdeaRatingView(APIView):
    ...
    def get(self, request, url_slug):
        ...
        if user_rating.exists():
            return Response({'user_rating': user_rating[0].rating, 'has_user_rated': True}, status=status.HTTP_200_OK)
        else:
            return Response({'user_rating': 0, 'has_user_rated': False}, status=status.HTTP_200_OK)