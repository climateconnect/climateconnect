...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        for badge in donor_badges:
            existing_badge = DonorBadge.objects.filter(regular_donor_minimum_duration=datetime.timedelta(days=badge['min_duration_in_days']))
            if existing_badge.exists():
                print("Badge for min duration {} days already existed. Skipping badge {}".format(badge['min_duration_in_days'], badge['name']))
            else:
                DonorBadge.objects.get_or_create(
                    name=badge['name'],
                    name_de=badge['name_de'],
                    image=badge['image'],
                    regular_donor_minimum_duration=datetime.timedelta(days=badge['min_duration_in_days']),
                    is_active=False
                )
                print("Inserted badge {}. Please set is_active to True in django admin to activate it.".format(badge['name']))
