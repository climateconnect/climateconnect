...
def create_skills():
    ...
    skills = [{'id':3,'name':'Communication & Marketing','parent_skill_id':None}
...
,{'id':131,'name':'Air and space transport expertise','parent_skill_id':130}
...
,{'id':135,'name':'Transport infrastructure expertise','parent_skill_id':130}
...
,{'id':151,'name':'Infrastructure','parent_skill_id':146}
    ...
    for skill_record in skills:
        if not Skill.objects.filter(name=skill_record['name']).exists():
                Skill.objects.create(id=skill_record['id'],
