    ...
    def handle(self, *args: Any, **options: Any) -> None:
            ...
            },
            {
                ...
                'answers': [
                    ...
                    },
                    {
                        ...
                        'language': "en",
                        'translations': {
                            'de': "Das könnte ich mir vorstellen"
                        },
                        'metadata': [
                            {
                                'resource_type': "organization",
        ...
        for question in questions:
            if not Question.objects.filter(step=question['step']).exists():
                answer_content_type = ContentType.objects.get(model=question['answer_type'])
                ...
                if "translations" in question:
                    for language_code in question['translations']:
                        ...
                        print("Inserted translation for question #{}".format(question['step']))
                ...
                if "answers" in question:
                    for answer in question['answers']:
                        ...
                        for piece_of_metadata in answer['metadata']:
                            ...
                            content_type = ContentType.objects.get(model=piece_of_metadata['resource_type'])
                        ...
                        answer_in_db = Answer.objects.create(
                            question=question_in_db, 
                            language=Language.objects.get(language_code=answer['language']), 
                            text=answer['text']
                        )
                        for metadata_db_entry in metadata_of_answer:
                            answer_in_db.answer_metadata.add(metadata_db_entry.id)
                        answer_in_db.save()
                        print("inserted answer for question #{}".format(question['step']))
                ...
                print("There was already a question for step {}".format(question['step']))
