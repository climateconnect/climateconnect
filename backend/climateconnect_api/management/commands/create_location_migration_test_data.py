...
def safe_bool(value_str):
    if not value_str:
        return None
    cleaned_value = value_str.strip().replace('“', '').replace('”', '').replace('"', '').lower()
...
#python manage.py create_location_migration_test_data.py --file <path_to_csv_table_with_test_database>