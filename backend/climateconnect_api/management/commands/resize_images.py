...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        user_profiles_to_fix = UserProfile.objects.filter(
            thumbnail_image__exact=""
        ).exclude(image="")
...
def fix_accounts(name, elements):
    print(
        "Total {name} to migrate to new image system: {number}".format(
            name=name, number=elements.count()
        )
    )
