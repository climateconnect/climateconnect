...
def create_follower_notification(notif_type_number, follower_type_field_name, obj_field_name, member_model, follower_type, follower_type_obj, follower_type_user_id):
    ...
    team = member_model.objects.filter(
        **team_query
    ).values("user")
    for member in team:
