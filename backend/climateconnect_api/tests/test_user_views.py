...
from climateconnect_api.tests.create_test_data import (create_project_test_data, create_org_test_data, create_idea_test_data)

from climateconnect_api.tasks import (fetch_entities_for_weekly_recommendations, fetch_user_info_for_weekly_recommendations, process_user_info_and_send_weekly_recommendations, fetch_and_create_globals_for_weekly_recommendations)

from django.urls import reverse

from climateconnect_api.factories import HubFactory, LanguageFactory, LocationFactory, UserFactory, UserProfileFactory
...
class TestRecommendedEmail(TestCase):
    ...
    def setUpTestData(cls):
        english_language = LanguageFactory(name='english', native_name='english', language_code='en', currency='$')
        german_language = LanguageFactory(name='german', native_name='Deutsch', language_code='de', currency='€')
        ...
        cls.english_user_in_hub_1 = UserProfileFactory(language=english_language, location=cls.location_1)
        cls.english_user_in_hub_2 = UserProfileFactory(language=english_language, location=cls.location_2)
        cls.german_user_in_hub_1 = UserProfileFactory(language=german_language, location=cls.location_1)
        cls.german_user_in_hub_2 = UserProfileFactory(language=german_language, location=cls.location_2)
        ...
        cls.english_user_in_hub_1_no_newsletter = UserProfileFactory(language=english_language, location=cls.location_1, send_newsletter=False)

        cls.project_in_hub_1_3likes = create_project_test_data(number_of_likes=3, location=cls.location_1, has_parent_organization=True)
        cls.project_in_hub_2_2likes = create_project_test_data(number_of_likes=2, location=cls.location_2)
        cls.project_in_hub_2_1likes = create_project_test_data(number_of_likes=1, location=cls.location_2, has_parent_organization=True)
        cls.project_in_hub_1_1likes = create_project_test_data(number_of_likes=1, location=cls.location_1)
        cls.project_outside_of_timespan = create_project_test_data(number_of_likes=5, location=cls.location_1, created_outside_of_timespan=True)
        cls.project_no_hub_4likes = create_project_test_data(number_of_likes=4, has_parent_organization=True)
        ...
        cls.org_in_hub_1_outside_of_timespan = create_org_test_data(location=cls.location_1, created_outside_of_timespan=True)

        cls.idea_in_hub_1 = create_idea_test_data(location=cls.location_1, hub=hub_1)
        cls.idea_in_hub_2_outside_of_timespan = create_idea_test_data(location=cls.location_2, hub=hub_2, created_outside_of_timespan=True)

        # creating user that receives all emails
        cls.admin_user = UserFactory(username='Admin', email=settings.MAILJET_ADMIN_EMAIL, first_name='You', last_name='Admin')
    ...
    def test_entities(self):   
        ...
        result = fetch_entities_for_weekly_recommendations(max_entities, timespan, self.location_1.id, is_in_hub)
        expected_result = [
        ...
        result = fetch_entities_for_weekly_recommendations(max_entities, timespan, self.location_2.id, is_in_hub)        
        expected_result = [
        ...
        is_in_hub = False
        no_location = 0
        result = fetch_entities_for_weekly_recommendations(max_entities, timespan, no_location, is_in_hub)        
        expected_result = [
    ...
    def test_user_data(self):
        ...
        result = {}
        is_in_hub = True
        result_queries = fetch_user_info_for_weekly_recommendations(self.location_1.id, is_in_hub)
        expected_result = {
        ...
        # for hub_2
        result = fetch_user_info_for_weekly_recommendations(self.location_2.id, is_in_hub)
        expected_result = {
    ...
    def test_send_email(self):
        ...
        for location_id in all_locations_in_hubs:
            ...
            lang_codes = list(Language.objects.values_list("language_code", flat=True).distinct())
            if mailjet_global_vars:
                for lang_code in lang_codes:
                    if (max_emails_sent < 1):
                        break
                    result = process_user_info_and_send_weekly_recommendations([self.admin_user.id], mailjet_global_vars, lang_code, is_in_hub, sandbox_mode)
