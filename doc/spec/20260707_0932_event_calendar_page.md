...
- **UI mockup**: A reference mockup is provided at [`event-calendar-mockup.png`](doc/spec/event-calendar-mockup.png) and defines the visual direction: a page title "Event calendar" with subtitle, a full-width **search bar at the top** (always visible), a **left filter panel** with Topic (select, "All topics"), Date From/To, and a Reset button, and the event list **grouped by day** with day headers (weekday + date). Each event uses a **wide card**: image on the left, content on the right (title, datetime range, location, and a status badge such as "Free"). This mockup is the source of truth for layout and should be matched during implementation.
...
- **Wide card**: Create a new `EventCardWide` component matching the mockup ([`event-calendar-mockup.png`](doc/spec/event-calendar-mockup.png)): horizontal layout with the image on the left and content on the right — title, datetime range, location, and a status badge (e.g. "Free" / registration state). Extract shared pieces (image, title, metadata, date indicator) from `ProjectPreview` into small shared subcomponents if it reduces duplication without complicating `ProjectPreview`.
...
- Add `EventCalendarContent` (or `EventCalendarPage`) that renders, inside `WideLayout`:
  ...
  - A full-width **search bar at the top** (always visible, matching the mockup), then a two-column layout: **left** = `FilterSection` (reuse existing component) configured with `sectors` (topic) and two date inputs (start/end), plus a Reset action; **right** = the event list grouped by day. On mobile, the topic/date filters collapse into a hamburger/drawer (reuse the Browse page's `MobileBottomMenu` / `filtersExandedOnMobile` pattern) while the text search bar remains always visible for quick search.
