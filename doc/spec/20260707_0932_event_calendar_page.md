...
- **Optional filter params** (mirror `ListEventsView` so the highlight matches the visible list): `search`, `sectors` (comma-separated), `hub`. The same exclusion (`is_draft=False`, `is_active=True`, `project_type=event`, `start_date__isnull=False`) and overlap semantics apply, scoped to the requested month (`start_date <= month_end AND (end_date IS NULL OR end_date >= month_start)`).
- **Response**: a JSON list of `{ "date": "YYYY-MM-DD", "count": <int> }` objects, one per day in the month that has at least one event, **sorted by date ascending**. Days with zero events are omitted. Multi-day events are counted on **every day they span** within the month (so a 3-day event contributes `count=1` to each of its three days).
...
- **Tests**: `backend/organization/tests/test_event_calendar_view.py::TestEventCalendarCountsView` covers 400-on-missing-params, invalid month, exclusion of draft/inactive/non-events, multi-day expansion, sorted output, and `search`/`sectors`/`hub` filtering.
...
- A custom `day` slot (`DayWithEvents`) renders the default `PickersDay` plus a small dot (`classes.eventDot`) under any day whose key is present in the map with `count > 0`. Days outside the current month are never dotted.
