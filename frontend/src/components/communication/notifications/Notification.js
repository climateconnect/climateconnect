...
import { IdeaCommentNotification, IdeaCommentReplyNotification, ProjectCommentNotification, ProjectCommentReplyNotification } from "./CommentNotifications";
...
export default function Notification({ notification, isPlaceholder }) {
  if (isPlaceholder) return <PlaceholderNotification />;
    ...
    if (type === "private_message")
      return (
        <PrivateMessageNotification notification={notification} />
      );
    else if (type === "group_message")
      return <GroupMessageNotification notification={notification} />;
    else if (type === "project_comment")
      return (
        <ProjectCommentNotification notification={notification} />
      );
      ...
      return (
        <ProjectFollowerNotification notification={notification} />
      );
...
const GroupMessageNotification = ({ notification }) => {
  ...
  return (
    <GenericNotification
      ...
      secondaryText={sender.first_name + " " + sender.last_name + ": " + notification.last_message.content}
    />
  );
};
...
const ProjectFollowerNotification = ({ notification }) => {
  const { locale } = useContext(UserContext)
  const texts = getTexts({ page: "notification", locale: locale });
  const followerName = notification.project_follower.first_name +" "+notification.project_follower.last_name
  return (
    <GenericNotification
