...
import { default as get_steps, default as tutorial_steps } from "../../../public/data/tutorial_steps";
  ...
  if(curStep.pointsAt) {
    return (
      <Popper        
        ...
        className={`${classes.popper} ${["bottom", "bottom-start", "bottom-end", "top", "top-start", "top-end"].includes(curStep.placement) && classes.vertical}`}
        modifiers={{
          preventOverflow: {
            enabled: false,
...
const Step = ({
  ...
  return (
      <div className={classes.root} ref={curStepRef}>
        <Typography component="h2" className={classes.headline}>{curStep.headline}</Typography>
        <Button onClick={onClickSkip} className={classes.closeButton}><CloseIcon /></Button>
        <StepText curStep={curStep} tutorialVariables={tutorialVariables} key={step}/>
        {step === 0 ?
          <div className={classes.buttonsContainer}>
            <Button className={classes.backwardButton} onClick={onClickSkip}>No</Button>
            <Button className={classes.forwardButton} onClick={onClickForward}>Yes!</Button>
          </div>
        :
        curStep.button ? 
          curStep.button
...
const ButtonBar = ({variableToSet, onClickForward, onClickBackward, possibleAnswers,forwardWithValue}) => {
  const classes = useStyles()
  return(
    <div className={classes.buttonsContainer}>
        ...
        <div className={classes.spreadOut}>
          {Object.keys(possibleAnswers).map(key => {
            return (
              <Button 
                key={key} 
                className={`${classes.forwardButton} ${Object.keys(possibleAnswers).indexOf(key) !== 0 && classes.marginLeft}`}
                onClick={() => forwardWithValue({
      ...
      :
        <div >
          <Button className={classes.backwardButton} onClick={onClickBackward}>Back</Button>   
          <Button className={classes.forwardButton} onClick={onClickForward}>Next</Button>   
        </div>
      }
    </div>
  )
