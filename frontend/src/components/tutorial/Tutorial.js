...
import { getLastCompletedTutorialStep, getTutorialStepFromCookie } from "../../../public/lib/tutorialOperations"
...
export default function Tutorial({
  ...
  const getNewCookieValue = (direction, step) => {
    ...
    if(direction === "forward"){
      if(oldCookieValue) {
        ...
        if(finishedSteps[finishedSteps.length-1] === step)
          return oldCookieValue
        else
          return oldCookieValue + "," + (step)
      } else {
        return step
      }
    } 
  ...
  const [step, setStep] = React.useState(tutorialCookie ? getTutorialStepFromCookie(tutorialSteps, tutorialCookie) : 0)
  const [showTutorial, setShowTutorial] = React.useState(false)
  const [showMinimizedAlert, setShowMinimizedAlert] = React.useState(false)
  const [tutorialVariables, setTutorialVariables] = React.useState(tutVarsCookie ? tutVarsCookie : {})
  useEffect(function() {
    if(step === 0){
      setTimeout(function() {
  ...
  const onClickSkip = () => {   
    ...
    cookies.set("lastStepBeforeSkipTutorial", getLastCompletedTutorialStep(finishedSteps), { path: "/", expires: oneYearFromNow, sameSite: "lax" })
    cookies.set("finishedTutorialSteps", "-1", { path: "/", expires: oneYearFromNow, sameSite: "lax" })
    setStep(-1)
  ...
  const resetSteps = () => {
    cookies.set("finishedTutorialSteps", "", { path: "/", expires: oneYearFromNow, sameSite: "lax" })
    setStep(0)
  }

  const handleClickForward = () => {
    const newCookieValue = getNewCookieValue("forward", step)
    cookies.set("finishedTutorialSteps", newCookieValue, { path: "/", expires: oneYearFromNow, sameSite: "lax" })
    setStep(getTutorialStepFromCookie(tutorialSteps, newCookieValue))
  }
  ...
  const forwardWithValue = ({variable, value}) => {
    ...
    cookies.set("finishedTutorialSteps", getNewCookieValue("forward", step), { path: "/", expires: oneYearFromNow, sameSite: "lax" })
    cookies.set("tutorialVariables", JSON.stringify(curTutorialVariables), { path: "/", expires: oneYearFromNow, sameSite: "lax" })
    setTutorialVariables(curTutorialVariables)
    setStep(getTutorialStepFromCookie(tutorialSteps, getNewCookieValue("forward", step)))
  } 
