 ...
 *
 * 7. Generate `devlink/componentRegistry.ts` by scanning all `.d.ts` files in the
 *    devlink directory (excluding `webflow_modules/` and infrastructure files).
 *    The registry maps each exported component name to its static import reference.
 *
...
// 7. Generate devlink/componentRegistry.ts.
// Scans all .d.ts files in devlink/ (excluding webflow_modules/ and infrastructure files),
// extracts exported component names via the `declare function` pattern, and generates
// a registry mapping component names to their static imports.
const INFRASTRUCTURE_FILES = new Set(["DevLinkProvider.d.ts", "ix3-interactions.d.ts"]);
const FUNCTION_DECL_REGEX = /^declare function (\w+)/m;
const registryEntries = [];

const dtsFiles = fs.readdirSync(devlinkDir).filter((f) => {
  if (!f.endsWith(".d.ts")) return false;
  if (f.startsWith("webflow_modules/")) return false;
  if (INFRASTRUCTURE_FILES.has(f)) return false;
  return true;
});

for (const dtsFile of dtsFiles) {
  const content = fs.readFileSync(path.join(devlinkDir, dtsFile), "utf8");
  const match = content.match(FUNCTION_DECL_REGEX);
  if (match) {
    const componentName = match[1];
    registryEntries.push({ componentName, fileName: dtsFile.replace(/\.d\.ts$/, "") });
  }
}
...
const registryLines = [];
registryLines.push(
  "// Auto-generated by scripts/fix-devlink.js — do not edit manually.",
  "// Run `node scripts/fix-devlink.js` after `npx webflow devlink sync` to regenerate.",
  ""
);

for (const { componentName, fileName } of registryEntries) {
  registryLines.push(`import { ${componentName} } from "./${fileName}";`);
}
...
console.log(
  `fix-devlink: generated componentRegistry.ts with ${registryEntries.length} components`
);
