...
async function newEmailVerification (uuid, token) {
    ...
    try {
        const response = await axios.post(
            process.env.API_URL + "/api/verify_new_email/", payload, tokenConfig(token)
        )
        redirect("/",{
        ...
        if(error.response && error.response.data) {
            if(error.response.data.detail)
                redirect("/",{'errorMessage': error.response.data.detail});
            else
            redirect("/",{'errorMessage': error.response.data.message});
