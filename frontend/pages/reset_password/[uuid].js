...
async function requestSetPassword(uuid, new_password,setErrorMessage) {
  ...
  try {
    const response = await axios.post(
      process.env.API_URL + "/api/set_new_password/", payload, config
    )
    redirect("/",{
    ...
    if(error.response && error.response.data) {
      if(error.response.data.type)
        setErrorMessage(<span>{error.response.data.message} <div><a href="/resetpassword">Click here to get another password reset email</a></div></span>)
      else
