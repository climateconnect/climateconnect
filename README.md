...
1. Wait for it to be done setting up the Dev Container and running the setup scripts. This will take 5-10 minutes the first time and 10 seconds after that. It should look something like this:
   ![](doc/devcontainer-done.png)
1. Start the frontend dev server using `cd frontend && yarn dev`
1. Start the backend server using `cd backend && python manage.py runserver`. If you get a "Django is not installed" error run `Ctrl+Shift+P` (or `Cmd+Shift+P`) "Select Python interpreter" -> Recommended so it uses the Python venv.
